...
const statusText = computed(() => {
  ...
  if (p && p.total > 0) {
    return t("fundSettings.recalcProgress", { done: p.done, total: p.total });
  }
  if (p) return t("fundSettings.recalcStart");
  ...
  <div class="space-y-4">
    <div>
      <label
        ...
        >{{ $t('fundSettings.currentCurrency') }}</label
      >
      <select
        :value="currency"
      ...
      >
        {{ $t('fundSettings.currencyReconvertHelp') }}
      </p>
    </div>
    ...
    >
      ...
      <div class="flex-1">
        <p>
          {{ $t('fundSettings.recalcFailed', { count: recalcFailedCount }) }}
        </p>
        <button
          @click="$emit('retry')"
